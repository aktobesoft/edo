...
from common_module.urls_module import approval_parameters, paginator_execute, query_parameters_list, query_parameters, query_parameters, task_parameters
from typing import Union

from documents.employee_task.models import EmployeeTaskNestedOut, EmployeeTaskNestedOutWithAllStatus, EmployeeTaskPUT, EmployeeTaskPOST, EmployeeTaskOut
...
async def get_employee_task_list(parameters: dict = Depends(query_parameters_list), 
                taskParameters: dict = Depends(task_parameters),current_user: UserModel = Depends(get_current_active_user)):
    await add_entity_filter(current_user, parameters)
    await add_parameters(taskParameters, parameters)
...
@employee_taskRouter.get('/{employee_task_id}',response_model = Union[EmployeeTaskNestedOutWithAllStatus, EmployeeTaskNestedOut, EmployeeTaskOut])
async def get_employee_task_by_id(employee_task_id : int, taskParameters: dict = Depends(task_parameters), 
...
@employee_taskRouter.post('/', response_model = Union[EmployeeTaskNestedOut, EmployeeTaskOut])
async def post_employee_task(employee_taskInstance : EmployeeTaskPOST, parameters: dict = Depends(query_parameters)\
    ...
    if parameters['nested']:
        return await views.get_employee_task_nested_by_id(result['id'], **parameters)
    else:
...
async def update_employee_task(employee_task_id: int, EmployeeTaskPUT : EmployeeTaskPUT, parameters: dict = Depends(query_parameters)\
    ...
    if parameters['nested']:
        return await views.get_employee_task_nested_by_id(employee_task_id, **parameters)
    else:
        return await views.get_employee_task_by_id(result['id'], **parameters)
